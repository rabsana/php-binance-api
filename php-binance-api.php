...
class API
{
    ...
    protected function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)
    {
        ...
        if ($signed === true) {
            ...
            curl_set_proxy($curl, $endpoint, array(
        ...
        elseif (count($params) > 0) {
            curl_set_proxy($curl, $this->getRestEndpoint() . $url . '?' . $query);
        }
        // no params so just the base url
        else {
            curl_set_proxy($curl, $this->getRestEndpoint() . $url, array(
                'X-MBX-APIKEY: ' . $this->api_key,
            ));
        }
        ...
        if (isset($json['msg']) && !empty($json['msg'])) {
            if ( $url != 'v1/system/status' && $url != 'v3/systemStatus.html' && $url != 'v3/accountStatus.html') {
                ...
                $code = 500;
                $matches = [];
                preg_match('/\"code\"\:\-\d{1,}\,/' , (string) $output , $matches);
                if(!empty($matches[0])){
                    preg_match('/\d{1,}/' , (string) $matches[0] , $numberMatches);
                    if(!empty($numberMatches[0])){
                        $code = (int) $numberMatches[0];
                    }
                }
                throw new \Exception('signedRequest error: '.print_r($output, true) , $code);
            }
        }
